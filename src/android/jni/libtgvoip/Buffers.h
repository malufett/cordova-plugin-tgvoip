...
namespace tgvoip
{
	...
	public:
		...
		{
			if (i >= length)
				return data[0]; // throw std::out_of_range("");
			return data[i];
		}
		const unsigned char &operator[](size_t i) const
		{
			if (i >= length)
				return NULL; // throw std::out_of_range("");
			return data[i];
		}
		...
		{
			if (!other.data)
				return; // throw std::invalid_argument("CopyFrom can't copy from NULL");
			if (other.length < srcOffset + count || length < dstOffset + count)
				return; // throw std::out_of_range("Out of offset+count bounds of either buffer");
		...
		void CopyFrom(const void *ptr, size_t dstOffset, size_t count)
		{
			if (length < dstOffset + count)
				return; // throw std::out_of_range("Offset+count is out of bounds");
